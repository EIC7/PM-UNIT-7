...
|---|---|---|
| v1.8 | 2026-08-15 | Lanjutan roll-out **Fitur O (Urutkan Foto) + Fitur R (Rotasi Gambar)** ke `ph-analyzer.html`: galeri `phImages[uid]` per unit. Fix bonus **Fitur Q** — kasus lebih rawan dari file-file sebelumnya: caption di sini TIDAK PUNYA `oninput` live-sync ke array sama sekali (beda dari opacity/dcs/dll yang setidaknya punya oninput sebagai fallback), sepenuhnya bergantung pada `phSyncCaptions()` yang dipanggil di awal render — begitu dipanggil ulang setelah `splice()` (hapus foto), sync itu baca DOM lama & tulis ke index array yang sudah bergeser → caption ketuker. Ditambahkan `oninput` langsung ke array (sekarang caption selalu live-sync, tidak cuma bergantung sync manual) + urutan sync-sebelum-splice + `skipSync` param, konsisten dengan pola file lain. |
